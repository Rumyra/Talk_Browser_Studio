...
class Grid {
	...
	get yGridSize() {
		...
		if (Array.isArray(this._distrib)) {
			...
			return this._calculateGridSize(this._distrib, 'y');
		}
	...
	getCoords(type) {
		...
		if (type === 'square') {
			...
			this._distrib.forEach( (el, i) => {
				coords.push( new Entity(el[0], el[1]) );
			} )
	...
	scaleGrid(vector = new Entity(1, 1)) {
	...
	randomiseGrid() {
		...
		this.coords.forEach( (el, i) => {
			let randomVecX = utils.randomNumber(-10, 10);
			let randomVecY = utils.randomNumber(-10, 10);
			let newVector = new Entity(randomVecX, randomVecY);
			el.add(newVector);
		} )
	}
